...
import type { TodayVM } from '../domain/view-models.ts';
import type { DomRenderer, RenderLabels } from './dom-renderer.ts';
...
export class DomController {
  ...
  #bindTodayHandlers(vm: TodayVM): void {
    if (vm.rest) return;
  ...
  #bindClose(btnId: string, modalId: string): void {
  ...
  #bindBackdrop(modalId: string): void {
    const m = this.#r.$(modalId);
    if (m) {
