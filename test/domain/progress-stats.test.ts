...
const PACK: Pack = {
  ...
  groups: [
    ...
    },
    {
      ...
      title: 'G2',
      phase: 'p2',
      items: [{ id: 'g2i1', track: 'dsa', tasks: [{ id: 't1', text: 'x' }] }],
    },
  ],
};
function complete(p: Progress, item: Item): void {
  for (const t of item.tasks ?? []) p.setTaskDone(item, t.id, true, '2026-05-30', 10);
}
const itemOf = (id: string): Item =>
  PACK.groups.flatMap((g) => g.items).find((i) => i.id === id)! as Item;
...
test('completedGroups counts a fully-complete non-rest group; byPhase skips phaseless groups; countTasks by track is positive', () => {
  ...
  const p = Progress.empty();
  complete(p, itemOf('g1i1'));
  complete(p, itemOf('g1i2')); // all non-rest items of g1
    ...
    groups: [
      {
        id: 'gx',
        title: 'GX',
        items: [{ id: 'gxi', track: 'dsa', tasks: [{ id: 't1', text: 'x' }] }],
      },
    ],
  };
